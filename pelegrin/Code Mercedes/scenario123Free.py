...
output_file = "/home/magi/UAMdeconflictionMasterThesis/modelli/data/mercedesTDFree/"
...
file.write(";\nset passedFreeF:=\n")
for trip in schedule["trips"]:
  if (trip["uid"] not in A or trip["uid"] in AP or trip["uid"] in AP_NC ):
    continue
...
file.write(";\nset fixedFlights:=\n")
for trip in schedule["trips"]:
	if (trip["uid"] in AP or trip["uid"] in AP_NC ):
		for i in range(len(trip["waypoints"])):
			if (i==0):
				continue
			file.write(str(trip["uid"]) + " " + str(trip["waypoints"][i-1]) + " "+ str(trip["waypoints"][i]) + "\n")
