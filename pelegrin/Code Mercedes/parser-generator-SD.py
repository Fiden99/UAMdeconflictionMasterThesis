...
verbose=float(sys.argv[4]) # draw net? print outputs?
...
v_climb=1.5
# schedule parameters:
numBatches=int(sys.argv[5])
tripsPerBatch=int(sys.argv[6])
timeBatch=float(sys.argv[7])
min_trip_duration=7
	...
	for line in openfileobject:
		...
		if line.startswith('\\psdots'):
			...
			if level.group(0)=='dotstyle=*,': # cruise
				...
				nodesCoord[nNodes]=[float(vector_coord[0]),float(vector_coord[1]),1] # cruise level set to 1 min
			else: # FATO
				isFATO[nNodes]=True
				nFatos=nFatos+1
				...
				nodesCoord[nNodes]=[float(vector_coord[0]),float(vector_coord[1]),0] # FATO level set to 0
			# node type
			...
			if color =="qqzzqq": #green, inner junc, code 2
				...
				if isFATO[nNodes]: nodeType[nNodes]=3
				else: nodeType[nNodes]=0
			elif color == "blue": # blue, landing, code 1 or 4
				if isFATO[nNodes]: nodeType[nNodes]=4
				else: nodeType[nNodes]=1
			else:
		...
		# 2. Edges and skylanes
			...
			if line.startswith('\\psline[linewidth=2pt]{->}'): # edge
				...
				for node in range(nNodes):
					if found_u==False and nodesCoord[node][0]==float(vector_coord[0]) and nodesCoord[node][1]==float(vector_coord[1]):
						found_u=True
						u=node
					elif found_v==False and nodesCoord[node][0]==float(vector_coord[2]) and nodesCoord[node][1]==float(vector_coord[3]):
						found_v=True
						v=node
				if isFATO[u]==True or isFATO[v]==True:
					if nodeType[v]==3: edges.append((v,u))
					elif nodeType[u]==4: edges.append((v,u))
					else: edges.append((u,v))
				else:
					edges.append((u,v))
				nEdges=nEdges+1
			else: #skylane
				skylanes.append((float(vector_coord[0]),float(vector_coord[1]),float(vector_coord[2]),float(vector_coord[3])))
	...
	if nodeType[i]!=3 and nodeType[i]!=4: # node is not a FATO
		for j in G2.neighbors(i):
			for k in G2.neighbors(i):
				if j< k and (nodeType[j]!=3 and nodeType[j]!=4) and (nodeType[k]!=3 and nodeType[k]!=4):	
...
for batch in range(0,numBatches):
	...
	for trip in range(0,tripsPerBatch):
		...
		while d== o or trip_duration< min_trip_duration or trip_duration >max_trip_duration:
...
sepCruise_slacks=[] # keep the minimum of the slacks of separation constraints of the SD model for each batch
sepClimb_slacks=[] 

for batch in range(0,numBatches):
	# 4a ...Load AMPL model and....
	ampl = AMPL()
	ampl.read('../../implementation/strategic-skylane.mod')
	# .... pass trip requests to AMPL data for SD
	A=range(batch*tripsPerBatch,batch*tripsPerBatch+tripsPerBatch)+pending_from_previous_batch + inflight_from_previous_batch
	...
	if (pending_from_previous_batch + inflight_from_previous_batch): #at least one of these lists is not empty
		previous_orig=dict((k,orig_list[batch-1][k]) for k in (pending_from_previous_batch + inflight_from_previous_batch) if k in orig_list[batch-1])
		orig_list[batch].update(previous_orig)
		previous_dest=dict((k,dest_list[batch-1][k]) for k in (pending_from_previous_batch + inflight_from_previous_batch) if k in dest_list[batch-1])
	...
	for (uid,wp1,wp2) in tuples_linkPath[batch]:
		if uid in inflight_from_previous_batch and (nodeType[wp1]!=3 and nodeType[wp2]!=4):
			const8=ampl.getConstraint('tmin_cruise')
			const9=ampl.getConstraint('tmax_cruise')
			const8[uid,wp1,wp2].drop()
			const9[uid,wp1,wp2].drop()
		elif uid in inflight_from_previous_batch and (nodeType[wp1]==3 or nodeType[wp2]==4):
	...
	ampl.setOption('presolve_eps',0.0000000000136) # this helps to avoid issues with separation constraints of 2 fixed trips from previous batch
	...
	if verbose:
		...
		sys.stdout.write('new scheduled trips: %f \n'%(int(var_numScheduled_nonpriority.value())-len(inflight_from_previous_batch)))
		sys.stdout.write('old scheduled trips: %f \n'%var_numScheduled_priority.value())
		sys.stdout.write('not scheduled trips: %f \n'%(len(A)-var_numScheduled_priority.value()-var_numScheduled_nonpriority.value()))
	...
	if verbose:
	 	print(slack_crossA, slack_crossB, slack_crossC, slack_merge, slack_split, slack_trailIni, slack_trailEnd) # cruise slack
	 	print(slack_mergeLand, slack_splitTakeoff, slack_trailLand, slack_trailTakeoff) # climb slack
	sepClimb_slacks.append(min(slack_mergeLand, slack_splitTakeoff, slack_trailLand, slack_trailTakeoff))
	sepCruise_slacks.append(min(slack_crossA, slack_crossB, slack_crossC, slack_merge, slack_split, slack_trailIni, slack_trailEnd))
