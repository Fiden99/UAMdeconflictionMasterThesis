...
void readMatrix(std::ifstream& file, Graph::Graph& graph, const int nn) {
	...
	for (int i = 0; i < nn; ++i)
	{
		...
		for (int i = 0; i < nn; ++i)
		{
			...
			graph.nodes[rowIndex]->outArcs.push_back(new Graph::Edge{ graph.nodes[rowIndex], graph.nodes[i], distance });
			graph.nodes[i]->inArcs.push_back(new Graph::Edge{ graph.nodes[rowIndex], graph.nodes[i], distance });
