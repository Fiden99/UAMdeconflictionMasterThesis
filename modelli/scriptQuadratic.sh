#!/bin/bash
#SBATCH --job-name=quadraticHeuristicUAM_array_job       # Nome del job
#SBATCH --output=/home/magi/UAMdeconflictionMasterThesis/modelli/out/nn3nf100/15/ampl_output_%A_%a.txt  # File di output per stdout (%A = job ID, %a = array task ID)
#SBATCH --error=/home/magi/UAMdeconflictionMasterThesis/modelli/out/nn3nf100/15/ampl_error_%A_%a.txt    # File di output per stderr
...
value="15" nNodes="3" nFlights="100" absPath=$PWD datFile=$datFileBase ampl /home/magi/UAMdeconflictionMasterThesis/modelli/quadraticHeuristic.run
