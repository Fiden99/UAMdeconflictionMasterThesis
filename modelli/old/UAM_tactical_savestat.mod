...
subject to defineT_down{f in freeF ,y in V: y <> s[f] and (f,y) not in (passedFreeNode union firstNotPassedFree)}:
t_down[f,y]=sum{(x,y) in E} w[x,y,f] *(d[x,y]/v_max + t_ear[f,x]);
subject to defineT_up{f in freeF ,y in V: y <> s[f] and (f,y) not in (passedFreeNode union firstNotPassedFree)}:
t_up[f,y]=sum{(x,y) in E} w[x,y,f] * (d[x,y]/v_min + t_ear[f,x]);

subject to defineT_down_fixed{(f,x,y) in fixedFlights :(f,x) not in timeFixed and f not in (AP union NC)}:
t_down_fixed[f,y]=d[x,y]/v_max + t_ear_fixed[f,x];
subject to defineT_up_fixed{(f,x,y) in fixedFlights : (f,x) not in timeFixed and f not in (AP union NC)}:
...
subject to fixFreeEar{(f,x) in passedFreeNode union firstNotPassedFree}:
t_ear[f,x] = t_hat_ear[f,x];
subject to fixFreeLat{(f,x) in passedFreeNode union firstNotPassedFree}:
t_lat[f,x] = t_hat_lat[f,x];
subject to fixFreePath{(f,x) in passedFreeNode union firstNotPassedFree, y in V : (f,x,y) in passedFreeF}:
w[x,y,f] = 1;
