...
#numConflicts[x,y,i,j] <=  wPath[x,y,i];
#subject to definenumConf2{(x,y) in E, i in F, j in F: i<>j}:
#numConflicts[x,y,i,j] <=  wPath[x,y,j];
#subject to definenumConf3{(x,y) in E, i in F, j in F: i<>j}:
#numConflicts[x,y,i,j] >=  wPath[x,y,i] + wPath[x,y,j] - 1;
...
t_up[f,y]=sum{x in V: (x,y) in E} (w[x,y,f] * (d[x,y]/v_min[f,x,y] + t_ear[f,x]));

# conflicts 

subject to trail1{i in F, j in F,(x,y) in E: i<>j}:
(v_min[i,x,y]*(t_ear[j,x]-t_lat[i,x]) >= D) or (v_min[j,x,y]*(t_ear[i,x]-t_lat[j,x])>=D);

subject to trail2{i in F, j in F,(x,y) in E: i<>j}:
(v_min[j,x,y]*(t_ear[j,y]-t_lat[i,y])>= D) or (v_min[i,x,y]*(t_ear[i,y]-t_lat[j,y])>= D);

subject to merge {i in F, j in F,x in V,(x1,x) in E, (x2,x) in E: i<>j and x1<>x2}: 
(t_ear[j,x]- t_lat[i,x]>=angleM[x,x1,x2]*D/v_min[j,x2,x]) or (t_ear[i,x]- t_lat[j,x]>=angleM[x,x1,x2]*D/v_min[i,x1,x]);

subject to diver {i in F, j in F,x in V,(x,x1) in E, (x2,x) in E: i<>j and x1<>x2}: #and x <> e[j]
(t_ear[j,x]- t_lat[i,x]>=anglePM[x,x1,x2]*(D/v_min[j,x2,x]+D/v_min[i,x,x1])) or (t_ear[i,x]- t_lat[j,x]>=anglePM[x,x1,x2]*(D/v_min[i,x2,x]+D/v_min[j,x,x1]));
#put ending cases

subject to split {i in F, j in F,x in V,(x,x1) in E, (x,x2) in E: i<>j and x1<>x2}:
(t_ear[j,x]- t_lat[i,x]>=angleP[x,x1,x2]*D/v_min[i,x,x1]) or (t_ear[i,x]- t_lat[j,x]>=angleP[x,x1,x2]*D/v_min[j,x,x2]);
